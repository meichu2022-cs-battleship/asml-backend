...
def mask_combine_then_dilate(image, mask, kernel_horizontal=(5, 1), kernel_vertical=(1, 5), show_process=False):
    # mask and contrast
    ...
    contrasted_mask = modify_contrast_and_brightness(
        masked, brightness=0, contrast=200)
    if show_process:
    ...
    if show_process:
        show_image(contrasted_mask_add_morph_horizontal_f,
    ...
    contrasted_mask_add_morph_vertical_f = np.copy(
        contrasted_mask_add_morph_vertical)
    if show_process:
        show_image(contrasted_mask_add_morph_vertical, title="vertical kernel")

    # combine all images
    contrasted_final = contrasted_mask_add + \
        contrasted_mask_add_morph_horizontal_f + contrasted_mask_add_morph_vertical_f
    contrasted_final[contrasted_final > 255] = 255
    if show_process:
        show_image(modify_contrast_and_brightness(
            contrasted_final, 0, 200), title="all added together")

    # compare
    if show_process:
        show_image(concat_images(contrasted_final, contrasted_mask_add),
                   title="compare image with one before dilation")

    # for display
    concat = concat_images(overlap_images(
        mask, contrasted_final, 0.5, 0.5), image)
    if show_process:
...
def mask_dilate_then_combine(image, mask, kernel_horizontal=(5, 1), kernel_vertical=(1, 5), show_process=False):
    # mask and contrast
    masked = mask_images(image, mask)
    masked_inv = mask_images(image, mask, invert=True)
    kernel_horizontal = np.ones(kernel_horizontal, np.uint8)
    ...
    # mask contrast
    contrasted_mask = modify_contrast_and_brightness(
        masked, brightness=0, contrast=225)
    ...
    # combine all images
    contrasted_mask = contrasted_mask + \
        contrasted_mask_horizontal + contrasted_mask_vertical
    ...
    if show_process:
        show_image(concat_images(contrasted_mask_before, contrasted_mask,
                                 contrasted_mask_morphed), title="Compared_all")
  ...
  # mask contrast inverted (We do not dilate the inverted masked image)
    ...
    # compare
    if show_process:
        show_image(concat_images(contrasted_without_dilation, contrasted_mask_final),
                   title="compare image with one before dilation")

    # # for display
    concat = concat_images(overlap_images(
        mask, contrasted_mask_final, 0.5, 0.5), image)
    if show_process:
        show_image(concat, "compare with sample")
...
def findDefectTu(detect, min_size=4, max_size=1024):
    ...
    nb_blobs, im_with_separated_blobs, stats, _ = cv2.connectedComponentsWithStats(detect)
...
def drawRectangle(im_result, shift=10, color_column=2):
    ...
    for i in range(len(contours)):
        ...
        cv2.rectangle(drawing, (int(boundRect[i][0])-shift, int(boundRect[i][1])-shift), \
          (int(boundRect[i][0]+boundRect[i][2])+shift, int(boundRect[i][1]+boundRect[i][3])+shift), color, -1)
    ...
    for i in range(len(contours)):
        ...
        #cv2.drawContours(drawing, contours_poly, i, color)
        cv2.rectangle(drawing, (int(boundRect[i][0])-shift, int(boundRect[i][1])-shift), \
          (int(boundRect[i][0]+boundRect[i][2])+shift, int(boundRect[i][1]+boundRect[i][3])+shift), color, 2)
        #cv2.circle(drawing, (int(centers[i][0]), int(centers[i][1])), int(radius[i]), color, 2)
    return drawing
...
def detect_sag(sem, gds):
    ...
    # 分界線 與 外部 二值化成白色
    # 內部 二值化成黑色
    th, boundary_external_white = cv2.threshold(
        sem_mask, 65, 255, cv2.THRESH_BINARY)
    ...
    # 將兩張疊合
    # 產生出只有缺陷為白色
    sag = cv2.bitwise_and(boundary_external_white,
                          boundary_external_white, mask=boundary_black)
    for i in range(3):
    ...
    # 畫框框
    rect = drawRectangle(cv2.cvtColor(dic['sag'], cv2.COLOR_BGR2GRAY))
    dic['rect'] = rect
    return dic
...
def detect_convex(sem, gds):
    ...
    '''
    drawing = drawRectangle(edges, color_column=1)
...
def detect_defect(image, ground_truth, output, limit=6):
    ...
    for i in range(1, image.shape[0] - 1):
        ...
        for j in range(1, image.shape[1] - 1):
            if output[i, j] == 255:
                continue
            if (ground_truth[i:i+limit, j] == 0).all() and (ground_truth[i:i+limit, j + 1] == 255).all():
                index = j + 1
                while index < image.shape[1] and (ground_truth[i:i+limit, index] == 255).all():
            ...
            if (ground_truth[i, j:j+limit] == 0).all() and (ground_truth[i+1, j:j+limit] == 255).all():
                index = i + 1
                while index < image.shape[0] and (ground_truth[index, j:j+limit] == 255).all():
...
def process_images(sem3d, gds3d) -> dict:
    ...
    return_dic["sem_gds_g_rect"] = rect_mask(return_dic["sem_gds"], process_dic["convex_rect"])
    return_dic["sem_gds_rg_rect"] = rect_mask(return_dic["sem_gds_r_rect"], process_dic["convex_rect"])
    return_dic["sem_r_rect"] = rect_mask(sem3d, process_dic["rect"])
    return_dic["sem_g_rect"] = rect_mask(sem3d, process_dic["convex_rect"])
    return_dic["sem_rg_rect"] = rect_mask(return_dic["sem_r_rect"], process_dic["convex_rect"]) 
